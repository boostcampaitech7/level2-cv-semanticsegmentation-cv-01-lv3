...
model = dict(
    ...
    backbone=dict(
        ...
        embed_dims=64,
        num_heads=[1, 2, 5, 8],
        num_layers=[3, 4, 6, 3]),  # B2 구조에 맞게 변경
    decode_head=dict(
...
#     dict(
#         type='LinearLR', start_factor=1e-6, by_epoch=False, begin=0, end=2000),  # warmup 구간


# training schedule
train_cfg = dict(type='IterBasedTrainLoop', max_iters=20000, val_interval=200)  # max_iters 100K로 설정
