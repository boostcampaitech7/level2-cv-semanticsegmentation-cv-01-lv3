...
class LoadXRayAnnotations(BaseTransform):
    ...
    def transform(self, results):
        ...
        label = np.zeros((image_size[0], image_size[1], len(self.classes)), dtype=np.uint8)
        ...
        try:
            ...
            results["gt_seg_map"] = np.zeros((image_size[0], image_size[1], len(self.classes)), dtype=np.uint8)
            results["seg_fields"] = ["gt_seg_map"]
...
class TransposeAnnotations(BaseTransform):
    def transform(self, results):
        # numpy array 상태에서 transpose만 수행
