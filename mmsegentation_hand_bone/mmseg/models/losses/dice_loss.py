...
def dice_loss(pred: torch.Tensor,
              ...
              class_weight: Union[torch.Tensor, None] = None,  # 클래스 가중치 추가
              eps: float = 1e-3,
    ...
    if class_weight is not None:
        ...
        # 각 클래스별 loss에 가중치 적용
        loss = loss * class_weight.view(1, -1).mean(dim=1)
...
class DiceLoss(nn.Module):
    ...
    def __init__(self,
                 ...
                 eps=1e-3,
                 loss_name='loss_dice',
                 class_weight: Union[torch.Tensor, None] = None):
        """Compute dice loss.
        ...
        Args:
            ...
            class_weight (torch.Tensor, optional): The weight of loss for each
                prediction, has a shape (n,). Defaults to None.
        """
