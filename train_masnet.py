...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='MANet',
                        help='모델 이름')
    ...
    parser.add_argument('--wandb_entity', type=str, default='cv01-HandBone-seg',
                        help='Wandb 팀/조직 이름')
    parser.add_argument('--wandb_run_name', type=str, default='PSPnet_efficientb0', help='WandB Run 이름')
...
def main():
    ...
    model = smp.PSPNet(encoder_name = 'efficientnet-b0',
                       encoder_weights = "imagenet",
                       encoder_depth = 3 ,
                       psp_output_channels = 512 ,
                       in_channel = 3 , 
                       classes  = len(CLASSES))
