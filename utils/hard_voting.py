...
if __name__ == "__main__":
    # 후보 모델 경로 설정
    csv_paths = ['../prediction/1024_lr2e4_6epoch_adamW.csv',
             '../prediction/dice_05-832_deeplabv3plus.csv',
             '../prediction/dice_05-efficientnet-b7_output.csv']

    # 투표 임계값 설정
    threshold = 2

    # 최종 ensemble output 저장 경로 설정
    save_dir = f"ensemble_threshold_{threshold}.csv"

    csv_ensemble(csv_paths, save_dir, threshold)