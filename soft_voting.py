...
class EnsembleDataset(Dataset):
    ...
    def __init__(self, fnames, cfg, transforms_dict):
        ...
        # 클래스 이름과 인덱스 매핑
        self.class2ind = {v: i for i, v in enumerate(cfg.CLASSES)}
...
def load_models(cfg, device):
    ...
    for  models_name in cfg.model_paths.keys(): # model_name , 
            ...
            for image_dict, image_names in data_loader:
                ...
                for model_name, models in model_dict.items():
                    for model in models:
                        ...
                        outputs = F.interpolate(outputs, size=(2048, 2048), mode="bilinear", align_corners=True)
                        outputs = torch.sigmoid(outputs)
                        total_output += outputs
                
                # 평균 계산 및 임계값 적용
                total_output = total_output / model_count
                total_output = (total_output > cfg.threshold).float()
